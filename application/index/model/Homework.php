...
class Homework extends Model
{
    ...
    //学生交作业方法
    public function pushwork($data, $user)
    {
        $show = db('homework')
                ->where('hid', $data['homeworkid'])
                ->select();

        $newdata['class_id'] = $user['class'];
        $newdata['stu_id'] = $_SESSION['think']['uid'];
        $newdata['dowork'] = $show[0]['hid'];
        $newdata['content'] = $data['content'];
        
        $result = db('homework')->insertGetId($newdata);
    }
